...
class MysqliDb
{
    ...
    )
    {
        ...
        if (is_object($host)) {
            $this->_mysqli = $host;
        } else // in case of using socket & host not exists in config array
        {
            if (is_string($host)) {
                $this->host = $host;
            }
    ...
    public function connect()
    {
        ...
        if ($this->_mysqli->connect_error) {
            throw new \Exception(
                'Connect Error ' . $this->_mysqli->connect_errno . ': ' . $this->_mysqli->connect_error,
    ...
    protected function reset()
    {
        if ($this->traceEnabled) {
            ...
            \Ap::debug("ExecSql:" . $this->trace[0][0] . "|usetime:" . $this->trace[0][1] . "|locat:" . $this->trace[0][2]);
        }
    ...
    private function queryUnprepared($query)
    {
        ...
        if (!$stmt) {
            throw new \Exception(
                "Unprepared Query Failed, ERRNO: " . $this->mysqli()->errno . " (" . $this->mysqli()->error . ")",
                $this->mysqli()->errno
            );
        };
    ...
    public function query($query, $numRows = null)
    {
        ...
        $stmt->execute();
        \Ap::debug(__METHOD__ . print_r($stmt, true) . "<-queryStmt");
        $this->_stmtError = $stmt->error;
        $this->_stmtErrno = $stmt->errno;
        $res = $this->_dynamicBindResults($stmt);
        $this->reset();
    ...
    protected function _buildQuery($numRows = null, $tableData = null)
    {
        ...
        \Ap::debug(__METHOD__ . print_r($this->isSubQuery, true) . ":QuerySql:" .$this->_query);
        if ($this->isSubQuery) {
            return;
        }
        ...
        // Prepare query
        $stmt = $this->_prepareQuery();
        \Ap::debug(__METHOD__ . print_r($stmt, true));
